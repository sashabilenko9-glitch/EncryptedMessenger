...
- **Protocol version** is exchanged in the handshake and in discovery announcements. A peer running another version (e.g. v1.0.0) is refused with a clear message ("bitte beide aktualisieren") in the chat, on request cards and in "In der Nähe", instead of failing silently. There is no negotiation, so a tampered version number can only make a connection fail, never select weaker crypto.

> **Note:** This is a study project. The very first connection to a contact is trusted automatically (TOFU). An attacker who intercepts that first connection, and every one after it, stays undetected unless the users compare the verification code. As soon as the real peer connects directly, the key mismatch blocks and flags it. UDP discovery announcements themselves are not authenticated, so a spoofed one can point a contact to a wrong address and make a connection attempt fail (it can't rename a contact: announced names only fill in placeholder names). It is intended for confidential messaging inside a trusted local network.
